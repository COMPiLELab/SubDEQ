...
path = "data_graph"
parser.target_path = path+"/citeseer/labels.pickle"
x = pd.read_pickle(path+"/citeseer/features.pickle").toarray()
hyper = pd.read_csv(path+"/citeseer/hyperedges-citeseer.txt",sep=", ", header=None)
n = x.shape[0]
parser.knownlabels = 0.063
...
graph = train.graph_reader(hyper,n)
test_nodes = train.test_split(0.042,graph)
