...
path = "data_graph"
parser.target_path = path+"/dblp/node-labels-dblp.txt"
x = pd.read_pickle(path+"/dblp/features.pickle").toarray()
hyper = pd.read_csv(path+"/dblp/hyperedges-dblp.txt",sep=", ", header=None)
n = x.shape[0]
parser.knownlabels = 0.06
...
graph = train.graph_reader(hyper,n)
test_nodes = train.test_split(0.04,graph)
