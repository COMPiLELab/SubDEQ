...
def test_split(knownlabels,graph):
        """
        Creating a train/test split.
        """
        nodes = [node for node in range(graph.number_of_nodes())]
        random.shuffle(nodes)

        train_size = int(np.ceil(len(nodes)*knownlabels))
        test_size = int((len(nodes)-train_size)/2)
        random.shuffle(nodes)

        test_nodes = nodes[:test_size]
        return test_nodes
...
class APPNPModel(torch.nn.Module):
    ...
    def forward(self, feature_indices, feature_values):
        ...
        elif self.args.model == "APPNP tanh inside":
            localized_predictions = latent_features_2
            edge_weights = torch.nn.functional.dropout(self.edge_weights,
                                                       p=self.args.dropout,
                                                       training=self.training)
...
class APPNPTrainer(object):
    ...
    """
    def __init__(self, args,graph, features, target,test_nodes,number_of_features):
        """
        :param args: Arguments object.
        :param graph: Networkx graph.
        :param features: Feature matrix.
    ...
    def train_test_split(self):
        ...
        nodes = list(set([node for node in range(self.graph.number_of_nodes())])-set(self.test_nodes))
    ...
    def fit(self):
        ...
        return self.best_accuracy,self.test_accuracy